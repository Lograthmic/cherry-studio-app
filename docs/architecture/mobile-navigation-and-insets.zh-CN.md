...
短的局部 picker，例如模型设置选择，使用由功能模块持有的可复用 Expo UI
`BottomSheet` 组件。这类 sheet 是由局部状态控制的普通 overlay；触发入口只传递
open/close 和选择状态。

带搜索/分组/说明的长列表 picker，默认使用 Expo Router route-level `formSheet`。这类
sheet 是 navigation stack 顶层 route，不是普通 overlay；Android 系统返回应该先
dismiss/pop sheet，再返回底层页面。
