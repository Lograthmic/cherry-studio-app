...
import { MODALITY, MODEL_CAPABILITY, type Modality } from '../../../schemas/enums';
import {
...
export const MODEL_TO_PUBLISHER: [RegExp, string][] = [
  ...
  [
    /^(gpt-|o1|o3|o4|chatgpt|dall-e|whisper|tts-|sora|text-embedding-ada|text-embedding-3|babbage|davinci)/,
    'openai',
  ],
  // Google models (including text-embedding-004, text-embedding-005)
  [
    /^(gemini|palm|gemma|veo|imagen|learnlm|text-embedding-00|text-multilingual-embedding-00|nano-banana)/,
    'google',
  ],
...
const REASONING_EXCLUDE =
  /\b(embed|rerank|dall-e|stable-diffusion|whisper|tts-|sdxl|flux|cogview|imagen)\b/i;
 ...
 *   2. Provider-level overrides (generate-providerModels.ts) for OpenAI/Anthropic/Google
...
const COMPUTER_USE_EXCLUDE =
  /\b(?:embed|rerank|tts-|dall-e|stable-diffusion|sdxl|flux|cogview|imagen|whisper)\b/i;
...
export const CAPABILITY_PATTERNS: [RegExp, RegExp | null, ModelCapabilityType][] = [
  ...
  [
    /(dall-e|stable-diffusion|sd3|sdxl|flux|image|imagen|midjourney|ideogram)/,
    null,
    MODEL_CAPABILITY.IMAGE_GENERATION,
  ],
  // Video generation models
  [
    /(sora|runway|pika|kling|veo|luma|gen-3|video|vidu|wan)/,
    null,
